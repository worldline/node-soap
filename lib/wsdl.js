  ...
  if (this.definitions.schemas) {
    ...
    if (schema) {
      if (parameterType.indexOf(':') !== -1) {
        var tmpXlmns = this.definitions.xmlns[parameterType.substring(0, parameterType.indexOf(':'))];
        parameterType = parameterType.substring(parameterType.indexOf(':') + 1, parameterType.length);
        // check in the right schema
        if(tmpXlmns in this.definitions.schemas) {
          schema = this.definitions.schemas[tmpXlmns];
        }
      }
