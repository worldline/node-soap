      ...
      if (!(typeName in definitions.descriptions.types)) {
        ...
        if (this.$ref) {
          element = definitions.descriptions.types[typeName];
            ...
            && type.namespace in xmlns
            && definitions
            && definitions.schemas
            && definitions.schemas[xmlns[type.namespace]]
            && definitions.schemas[xmlns[type.namespace]].complexTypes
            && definitions.schemas[xmlns[type.namespace]].complexTypes[typeName]) {
          element[name] = definitions.schemas[xmlns[type.namespace]].complexTypes[typeName];
          element[name] = element[name].description(definitions, xmlns);
        }
        else {
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  if (root.Envelope) {
    ...
    if (body.Fault) {
      ...
      var detail = selectn('detail.$value', body.Fault) || selectn('detail', body.Fault);
...
WSDL.prototype.findParameterObject = function(xmlns, parameterType) {
  ...
  if (this.definitions.schemas) {
    ...
    if (schema) {
      var childNamespace, childXmlns;
      if (parameterType.indexOf(':') !== -1) {
        childNamespace = parameterType.substring(0, parameterType.indexOf(':'));
        ...
        // check in the right schema
        if(childXmlns in this.definitions.schemas) {
          schema = this.definitions.schemas[childXmlns];
        }
      }
      ...
      parameterTypeObj = schema.complexTypes[parameterType] || schema.elements[parameterType] ||
        _.find(schema.elements, {'$lookupType': parameterType });

      if(parameterTypeObj && childNamespace && childXmlns) {
        parameterTypeObj.childNameSpace = childNamespace;
        parameterTypeObj.childXmlns = childXmlns;
      }
    }
  }
...
/**
 * Return the object keys in the right order according to the schema (for sequences)
 * @param {Object} obj - object to convert
 * @param {Object} parameterTypeObject - schema of the current element
 * @param {Boolean} first - is root element
 * @return {Arrray} - an ordered array of the attributes names
 */
WSDL.prototype.getAttributesNamesInOrder = function(obj, parameterTypeObject, first) {
  var self = this;
  if(! _.isObject(obj)) {
    return [];
  }
  var keys = Object.keys(obj);
  var tmpParameterTypeObject = parameterTypeObject;
  if(tmpParameterTypeObject && tmpParameterTypeObject.name === 'element') {
    tmpParameterTypeObject = _.find(tmpParameterTypeObject.children, { name: 'complexType' });
  }
  // Check if it's a sequence
  if(! first && tmpParameterTypeObject && tmpParameterTypeObject.name === 'complexType') {
    var sequence = _.cloneDeep(_.find(tmpParameterTypeObject.children, { name: 'sequence' }));
    // It's a sequence, reorder elements with schema information
    if(sequence) {
      // it's a choice
      sequence = _.find(sequence.children, { name: 'choice' }) || sequence;

      var elemSchema;
      // Case of xsi_type
      if (obj[self.options.attributesKey] && obj[self.options.attributesKey].xsi_type) { //if parent object has complex type defined and child not found in parent
        elemSchema = self.findChildParameterObjectFromSchema(obj[self.options.attributesKey].xsi_type.type, obj[self.options.attributesKey].xsi_type.xmlns);
        if(elemSchema) {
          var tmpElem = _.find(elemSchema.children, { name: 'complexContent' });
          if(tmpElem) {
            tmpElem = _.find(tmpElem.children, { name: 'extension' });
            if(tmpElem) {
              elemSchema = _.find(tmpElem.children, { name: 'sequence' }) || elemSchema;
            }
          }
        }
      }

      if(_.isArray(sequence.children) && elemSchema && _.isArray(elemSchema.children)) {
        sequence.children = sequence.children.concat(elemSchema.children);
      }

      keys = _.reduce(sequence.children, function(result, elem) {
        if(_.includes(keys, elem.$name)) {
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  if (Array.isArray(obj)) {
    ...
    //Reorder elements if it's a sequence
    var attributeNames = self.getAttributesNamesInOrder(obj, parameterTypeObject, first);
    ...
    for (var j = 0; j < attributeNames.length; ++j) {
      ...
      var nonSubNameSpace = '';
      if (first) {
        var childNamespaceFirst = (parameterTypeObject && parameterTypeObject.childNameSpace) ? parameterTypeObject.childNameSpace : namespace;
        value = self.objectToXML(child, name, childNamespaceFirst, xmlns, false, null, parameterTypeObject, ancXmlns);
      } else {
        ...
        if (self.definitions.schemas) {
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              if(childParameterTypeObject.$baseNameSpace && childParameterTypeObject.$baseNameSpace !== namespace ) {
              ...
              if(childNamespace && !childXmlns && parameterTypeObject.childXmlns) {
                var tns = parameterTypeObject.childXmlns;
                if(self.definitions.schemas[tns] && self.definitions.schemas[tns].xmlns ) {
                  childXmlns = self.definitions.schemas[tns].xmlns[childNamespace];
                }
              }
...
WSDL.prototype.findChildParameterObjectFromSchema = function(name, xmlns) {
  ...
  return schema.complexTypes[name] || schema.types[name] || schema.elements[name];
};
...
var WSDL_sync = function(definition, path, options){
  this._initializeOptions(options);
  definition = stripBom(definition);
...
util.inherits(WSDL_sync, WSDL);

function open_wsdlSync(path, options){
  var definition = fs.readFileSync(path, "utf8");
  return new WSDL_sync(definition, path, options);
}
...
exports.open_wsdlSync = open_wsdlSync;
exports.WSDL = WSDL;
